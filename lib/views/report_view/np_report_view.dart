...
class NpReportView extends StatefulWidget {
  const NpReportView({super.key});

  @override
  State<NpReportView> createState() => _NpReportView();
}

// Map of warehouse names to user IDs
final Map<String, String> warehouseToUserIdMap = {
  "DF-Acer": "BF76-FE5F-6DD0-9FFD",
  "Asus_black": "5D3A-7D53-1EF7-67A1",
  "DF Asus Small": "BF76-FE5F-6DD0-9FFD",
  "DF Asus HDD": "4CAD-BDAD-478F-4B49",
};

// Map of warehouse names to user names
final Map<String, List<String>> warehouseToUserNamesMap = {
  "DF-Acer": ["Owner"],
  "Asus_black": ["owner", "Phyo Wai Sale 3", "admin", "STK Sale4"],
  "DF Asus Small": ["owner", "admin@fusionmyanmar.com"],
  "DF Asus HDD": [], // No users for DF Asus HDD
};

class _NpReportView extends State<NpReportView> {
  int? _reamaingDay;
  LastSubscriptionModel? _lastSubscriptionModel;
  String _selectedWarehouse = 'DF-Acer'; // Default warehouse selection
  String _selectedDateFilter = 'Today';
  String _selectedUserNameFilter = 'All'; // Default user name filter selection
  int? _selectedStarStatus;

  List<Map<String, dynamic>> starNPData = [];
  ...
  Future<void> _initializeData() async {
    ...
    await _loadNpDataFromFile(); // Load initial data
  }

  Future<void> _loadNpDataFromFile() async {
    try {
      // Load data from the JSON file (assuming the path is known)
      final directory = await getApplicationDocumentsDirectory();
      final file = File('${directory.path}/StarNP.json');
      if (file.existsSync()) {
        await _loadNpData(file);
      } else {
        log('StarNP.json file not found');
      }
    } catch (e) {
          ...
          child: ListView(
            children: [
              ...
              Row(
                children: [
                  Expanded(
                    child: _buildUserNameDropdown(),
                  ),
                  const SizedBox(width: 200), // Spacing between dropdowns
                  Expanded(
  ...
  Widget _buildUserNameDropdown() {
    // Get the user names based on the selected warehouse
    List<String> userNames = warehouseToUserNamesMap[_selectedWarehouse] ?? [];

    return DropdownButton<String>(
      value: _selectedUserNameFilter,
      onChanged: (String? newValue) {
        setState(() {
          _selectedUserNameFilter = newValue!;
        });
      },
      items: ['All', ...userNames]
  ...
  Widget _buildNetPurchaseList() {
    // Filter the data according to the selected date filter
    List<Map<String, dynamic>> filteredData = starNPData
        .where((item) => item['starFiler'] == _selectedDateFilter)
        .toList();

    // Additional filter for selected user name
    if (_selectedUserNameFilter != 'All') {
      filteredData = filteredData
          .where((item) => item['starUserName'] == _selectedUserNameFilter)
          .toList();
    }
    ...
    List<dynamic> starNPItemList = filteredData[0]['starNPItemList'];
        ...
        DropdownButton<String>(
          ...
          onChanged: (String? newValue) {
            setState(() {
              ...
              _selectedUserNameFilter = 'All'; // Reset user name filter on warehouse change
            });
          },
...
// Method to build the Total Amount Box
  Widget _buildTotalAmountBox() {
    ...
    if (filteredData.isNotEmpty) {
      ...
      totalInvoices = starNPItemList.length;
      totalAmount = starNPItemList.fold(0, (sum, item) => sum + (item['starAmount'] as num).toDouble());
      totalPaidAmount = starNPItemList.fold(0, (sum, item) => sum + (item['starPaidAmount'] as num).toDouble());
    }
...
// Helper method to create each row of the total box
  ...
  Future<void> _loadNpData(File file) async {
    try {
      // Read the file
