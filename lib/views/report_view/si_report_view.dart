...
class SiReportView extends StatefulWidget {
  const SiReportView({super.key});

  @override
  State<SiReportView> createState() => _SiReportView();
}
...
class _SiReportView extends State<SiReportView> {
  int? _reamaingDay;
  ...
  Widget _buildSaleItemList() {
    ...
    List<Map<String, dynamic>> filteredData = starSIData
...
// Method to build the Total Amount Box
  Widget _buildTotalAmountBox() {
    ...
    if (filteredData.isNotEmpty) {
      List<dynamic> starSIItemList = filteredData[0]['starItemList'];

      // Calculate the total invoices, total amount, and total paid amount
      totalQty = starSIItemList.fold(0, (sum, item) => sum + (item['starQty'] as num).toDouble());
      totalAmount = starSIItemList.fold(0, (sum, item) => sum + (item['starAmount'] as num).toDouble());
